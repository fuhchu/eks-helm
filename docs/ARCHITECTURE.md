...
### Secrets: AWS Secrets Manager via External Secrets Operator (not plaintext in values.yaml)
The Postgres password originated as plaintext in `values.yaml` — recoverable forever
from git history AND from Helm release history (each revision is itself a Secret
storing the full values). This was fixed in two layers:

1. **Kubernetes Secrets (the primitive):** credentials moved out of `values.yaml`
   into `Secret` objects referenced by `secretKeyRef`. Important caveat — a K8s
   Secret is only base64-**encoded**, not encrypted. Its value is defense via RBAC
   (who can `get secrets`) plus optional KMS envelope encryption on etcd, not the
   Secret object itself.
2. **AWS Secrets Manager (the source of truth):** the password is generated by
   Terraform (`random_password`), stored only in Secrets Manager, and pulled into
   the cluster by the External Secrets Operator (ESO). ESO's controller authenticates
   to AWS via IRSA (scoped to `GetSecretValue` on the one secret) and materializes
   native K8s Secrets, so the pods' existing `secretKeyRef` refs work unchanged.

The password never appears in git, `values.yaml`, a `--set` flag, or shell history.
It exists in exactly two places: Terraform state (encrypted in S3) and Secrets Manager.

Design detail: the full `DATABASE_URL` connection strings are assembled in Terraform
(where the password is known) and stored as extra keys in the Secrets Manager JSON,
so ESO maps keys 1:1 with no ESO-side templating — sidestepping ESO's Go-template
syntax colliding with Helm's.

### Password rotation and the StatefulSet's data limitation
`POSTGRES_PASSWORD` is only honored on **first init of an empty volume** — an existing
Postgres data directory keeps whatever password it was created with. During this build
the password was rotated by wiping the PVC and re-initializing, which is acceptable
ONLY because the data is disposable. In a live system that command is unrecoverable
data loss. Production rotation without data loss: `ALTER USER ... WITH PASSWORD` against
the running DB, RDS IAM database auth (no static password at all), or Secrets Manager
managed rotation. This is the same reason real data belongs in RDS (backups, Multi-AZ,
managed rotation), not a single-replica StatefulSet on EBS.
