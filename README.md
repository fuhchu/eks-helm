...
- **AWS Secrets Manager + External Secrets Operator** — DB password generated by Terraform, stored only in Secrets Manager, pulled into the cluster via IRSA; never in git or Helm values
- **Terraform** — VPC, EKS, ECR, IAM/IRSA, ACM, Secrets Manager, EKS Access Entries; plan/apply split with a manual approval gate
